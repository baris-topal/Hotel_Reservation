...
def create_app():
    ...
    #çıkış yap fonksiyonumuz kullanıcının sepetinin boşaltılmasını sağlıyor
    #ve sistemden kullanıcıyı çıkarıyor
    @app.route("/logout")
    @login_required
    def logout():
        conn = engine.connect()
    ...
    #formdan gelen verileri kullanarak veritabanına yeni
    #bir kullanıcı ekliyoruz
    @app.route('/', methods=['GET','POST'])
    def uye_ol():
        if request.method == 'POST':
            #şifreyi hashli bir şekilde tutmayı sağlıyoruz
    ...
    #adminin sistemde kayıtlı üyeleri görebilmesini sağlayan fonksiyon yazıyoruz
    @app.route('/uye_gor', methods=['GET','POST'])
    @login_required
    @admin_required
    def uye_gor():
        conn = engine.connect()
        #veritabanından sadece normal üyeleri çeken sorgumuzu yazıyoruz
    ...
    #tablodan gelen kullanıcı id si ile eşleşen kullanıcı
    #veritabanından silen fonksiyonu yazıyoruz
    ...
    #veritabanında varolan otelleri görüntülemeyi ve yeni otel eklemeyi sağlıyoruz
    ...
    #tablodan seçilen otelin düzenlemesini yapmak için formdan gelen verilerle
    #otelin bilgileri update sorgusu ile güncelliyoruz, eğer otel yoksa yani yeni ekleniyorsada ekleme işlemi yapıyoruz
    ...
    #tablodan gelen otel id si ile eşleşen oteli
    ...
    #tablodan seçilen otel id ile eşleşen otele oda ekliyoruz
    @app.route('/oda_listele', methods=['GET','POST'])
    @login_required
    @admin_required
    def oda_listele():
        if request.method=='POST':
            #oda eklemeyi sağlıyoruz
    ...
    #tabloda seçilen oda id ile eşleşen
    #oda bilgilerini formdan gelen verilerle güncelliyoruz
    ...
    #tablodan gelen oda id si ile eşleşen odayı
    #veritabanından siliyoruz    
    ...
    #oda durumu 0 olan yani müsait olan odaları 
    #rezervasyon yapabilmek için tabloda görüntülüyoruz
    ...
    #rezervasyon yapmak için seçtiğimiz odayı
    #sepete eklememizi sağlıyor
    ...
    #kullanıcı puanını ve rezervasyon yaptığmız oda bilgilerini görüntülememizi sağlıyor
    ...
    #tablodan seçilen otelin bilgilerini kullanmak için aracı bir fonksiyon yazıyoruz
    #ve sepet_onay_html sayfasına yönlendiriyoz
    @app.route('/sepet_tarih_sec/<id>/<odaid>',methods=['GET','POST'])
    @login_required
    def sepet_tarih_sec(id=None, odaid=None):

        return render_template("sepet_onay.html", sepet_id = id, oda_id = odaid)

    #sepetteki rezervasyon bilgilerini formdan alıyoruz ve güncelliyoruz
    #kullanıcının alışveriş durumunda kazanacağı puanı hesaplıyoruz
    #puan kullanması durumunda indirim uygulamayı sağlıyoruz
    ...
    #sepette ki rezervasyonumuzu onaylıyoruz 
    #onaylanan rezervasyon sepetten siliniyor ve 
    #işlemlerim kısmına düşüyor
    ...
    #sepette ki rezervasyonumuzu siliyoruz
    ...
    #kullanici id ile eşleşen geçmiş işlemleri görüntüluyoruz
